...
class Order extends \Magento\Backend\Block\Widget\Grid\Extended
{
     ...
     * @param \Lof\SalesRep\Model\ResourceModel\SalesrepOrder\CollectionFactory $salesrepOrderCollectionFactory
    ...
    public function __construct(
        ...
        \Lof\SalesRep\Model\ResourceModel\SalesrepOrder\CollectionFactory $salesrepOrderCollectionFactory,
        ...
        $this->salesrepOrderCollectionFactory    = $salesrepOrderCollectionFactory;
    ...
    {
        if ($column->getId() == 'assigned_user_order') {
    ...
    {
        if ($this->getRequest()->getParam('in_salesrep_order') != "") {
            return $this->getRequest()->getParam('in_salesrep_order');
        }
        ...
        $result = $this->salesrepOrderCollectionFactory->create()->addFieldToFilter('user_id', array('eq' => $userId))->load();
        $customerIds = [];
