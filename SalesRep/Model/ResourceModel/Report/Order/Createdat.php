<?php
namespace Lof\SalesRep\Model\ResourceModel\Report\Order;

/**
 * Class Createdat
 * @package Lof\SalesRep\Model\ResourceModel\Report\Order
 */
class Createdat extends \Magento\Sales\Model\ResourceModel\Report\Order\Createdat
{
    ...
    protected function _aggregateByField($aggregationField, $from, $to)
    {
        ...
        try {
            ...
            $columns['salesrep_id'] = 'salesrep_id';
            ...
            $select->group(['period', 'order_status', 'salesrep_id']);
            $connection->query($select->insertFromSelect($this->getMainTable(), array_keys($columns)));
            $connection->commit();
        } catch (\Exception $e) {
            $connection->rollBack();
